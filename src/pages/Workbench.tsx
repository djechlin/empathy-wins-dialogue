      ...
      <div className="p-6">
        <div className="max-w-7xl mx-auto">
          <div className={`grid grid-cols-1 lg:grid-cols-2 gap-6 ${!user ? 'filter blur-sm pointer-events-none' : ''}`}>
            ...
            <div className="space-y-4">
              {/* Organizer Selection Warning */}
              {(() => {
                const starredOrganizers = state.organizers.filter(o => o.starred);
                if (starredOrganizers.length > 1) {
                  return (
                      ...
                      <div className="flex items-start gap-3">
                        <div className="flex-shrink-0">
                          <svg className="h-5 w-5 text-yellow-600" fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d="M8.485 3.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.19-1.458-1.517-2.625L8.485 3.495zM10 6a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 6zm0 9a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" />
                          </svg>
                        </div>
                        <div>
                          <h3 className="text-sm font-medium text-yellow-800">Multiple Starred Organizers</h3>
                          <p className="text-sm text-yellow-700 mt-1">
                            Star the organizer you want to use. Only one organizer is supported at a time right now.
                            Currently starred: {starredOrganizers.map(o => o.name).join(', ')}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  return (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <h3 className="text-sm font-medium text-blue-800">No Organizer Selected</h3>
                          <p className="text-sm text-blue-700 mt-1">
                            Star an organizer to enable chat functionality.
                          </p>
                        </div>
                      </div>
                    </div>
