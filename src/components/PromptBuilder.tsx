...
const PromptBuilder = memo(
  forwardRef<PromptBuilderRef, PromptBuilderProps>(
    (
      {
        persona,
        color,
      ...
      useEffect(() => {
        ...
        } else if (!isDirty && state.saveStatus === SaveStatus.DIRTY) {
          dispatch({ type: 'SAVE_SUCCESS', payload: { id: state.id || '' } });
        }

        onDirtyChange?.(isDirty || state.saveStatus === SaveStatus.DIRTY);
      }, [
        isDirty,
        state.id,
        state.systemPrompt,
      ...
      useEffect(() => {
        if (
          ...
          initialFirstMessage === state.firstMessage &&
