...
import { fetchAllPromptBuildersForPersona, archivePromptBuilder, type PromptBuilderData } from '@/utils/promptBuilder';
...
const PromptBuilderSet = ({ persona, color, onSelectPrompt, onCreateNew }: PromptBuilderSetProps) => {
  ...
  return (
    <div className={`${color} rounded-lg p-4 space-y-4`}>
      {/* Header with persona name and create button */}
      <div className="flex items-center justify-between">
        <h3 className="font-medium text-lg">{persona.charAt(0).toUpperCase() + persona.slice(1)} Prompts</h3>
        {onCreateNew && (
