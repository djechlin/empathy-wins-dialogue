...
const MockDialogueContext = createContext<VoiceContextType | undefined>(undefined);

export const useMockDialogue = () => {
  const context = useContext(MockDialogueContext);
  if (!context) {
    throw new Error('useMockDialogue must be used within a MockDialogueProvider');
  }
  return context;
};
...
export function MockDialogueProvider({ children, className }: MockDialogueProviderProps) {
  ...
  return (
    <div className={className}>
      <MockDialogueContext.Provider value={mockContext}>
        {children}
      </MockDialogueContext.Provider>
    </div>
  );
}