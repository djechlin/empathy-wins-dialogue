...
const ScoreCard = ({
  ...
  return (
    <div className={cn('w-full p-3 rounded-lg transition-colors duration-200 flex items-start gap-3 relative', getCardStyles())}>
      {/* Icon */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1">
          <h4 className="font-medium text-base text-gray-800 font-sans">{config.title}</h4>
          {badgeContent && (
            <Badge variant="secondary" className={cn('text-xs ml-2 flex-shrink-0', getBadgeStyles())}>
              {badgeContent}
            </Badge>
          )}
        </div>
        ...
        {config.tip && !tipDismissed && (
          <div className="mb-3 bg-blue-50 border border-blue-200 rounded-lg p-3 relative">
            ...
            <div className="flex items-start gap-2 pr-6">
              ...
              <p className="text-sm text-blue-800">{config.tip}</p>
            </div>
          </div>
        )}
